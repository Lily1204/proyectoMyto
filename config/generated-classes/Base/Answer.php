...
abstract class Answer implements ActiveRecordInterface
{
    ...
    public function setRespuestaId($v)
    {
        ...
        if ($this->respuesta_id !== $v) {
            $this->respuesta_id = $v;
            $this->modifiedColumns[AnswerTableMap::COL_RESPUESTA_ID] = true;
        }
    ...
    public function setRespuesta($v)
    {
        ...
        if ($this->respuesta !== $v) {
            $this->respuesta = $v;
            $this->modifiedColumns[AnswerTableMap::COL_RESPUESTA] = true;
        }
    ...
    public function setPreguntaId($v)
    {
        ...
        if ($this->pregunta_id !== $v) {
            $this->pregunta_id = $v;
            $this->modifiedColumns[AnswerTableMap::COL_PREGUNTA_ID] = true;
        }

        if ($this->aQuestion !== null && $this->aQuestion->getPreguntaId() !== $v) {
            $this->aQuestion = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : AnswerTableMap::translateFieldName('RespuestaId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->respuesta_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AnswerTableMap::translateFieldName('Respuesta', TableMap::TYPE_PHPNAME, $indexType)];
            $this->respuesta = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AnswerTableMap::translateFieldName('PreguntaId', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    {
        if ($this->aQuestion !== null && $this->pregunta_id !== $this->aQuestion->getPreguntaId()) {
            $this->aQuestion = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[AnswerTableMap::COL_RESPUESTA_ID] = true;
        if (null !== $this->respuesta_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . AnswerTableMap::COL_RESPUESTA_ID . ')');
        }
        if (null === $this->respuesta_id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('respuestas_respuesta_id_seq')");
                $this->respuesta_id = (int) $dataFetcher->fetchColumn();
            } catch (Exception $e) {
        ...
        if ($this->isColumnModified(AnswerTableMap::COL_RESPUESTA_ID)) {
            $modifiedColumns[':p' . $index++]  = 'respuesta_id';
        }
        if ($this->isColumnModified(AnswerTableMap::COL_RESPUESTA)) {
            $modifiedColumns[':p' . $index++]  = 'respuesta';
        }
        if ($this->isColumnModified(AnswerTableMap::COL_PREGUNTA_ID)) {
            $modifiedColumns[':p' . $index++]  = 'pregunta_id';
        }

        $sql = sprintf(
            'INSERT INTO respuestas (%s) VALUES (%s)',
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'respuesta_id':
                        $stmt->bindValue($identifier, $this->respuesta_id, PDO::PARAM_INT);
                        break;
                    case 'respuesta':
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setRespuestaId($arr[$keys[0]]);
        }
        if (array_key_exists($keys[1], $arr)) {
            $this->setRespuesta($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setPreguntaId($arr[$keys[2]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AnswerTableMap::COL_RESPUESTA_ID)) {
            $criteria->add(AnswerTableMap::COL_RESPUESTA_ID, $this->respuesta_id);
        }
        if ($this->isColumnModified(AnswerTableMap::COL_RESPUESTA)) {
            $criteria->add(AnswerTableMap::COL_RESPUESTA, $this->respuesta);
        }
        if ($this->isColumnModified(AnswerTableMap::COL_PREGUNTA_ID)) {
            $criteria->add(AnswerTableMap::COL_PREGUNTA_ID, $this->pregunta_id);
        }
    ...
    {
        $validPk = null !== $this->getRespuestaId();
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setPreguntaId($this->getPreguntaId());
        if ($makeNew) {
            $copyObj->setNew(true);
    ...
    public function setQuestion(ChildQuestion $v = null)
    {
        if ($v === null) {
            ...
            $this->setPreguntaId($v->getPreguntaId());
        }
    ...
    {
        if ($this->aQuestion === null && ($this->pregunta_id != 0)) {
            $this->aQuestion = ChildQuestionQuery::create()->findPk($this->pregunta_id, $con);
